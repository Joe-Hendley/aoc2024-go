...
func solvePart1Iterative(input string) int {
	printQueue := newPrintQueue(input)
	return printQueue.sumMiddleOfCorrectUpdates()
}

func solvePart2Iterative(input string) int {
	printQueue := newPrintQueue(input)
...
func newPrintQueue(input string) printQueue {
	...
	for _, line := range strings.Split(parts[0], "\n") {
		...
		x := must.Atoi(ruleParts[0])
		y := must.Atoi(ruleParts[1])
	...
	for _, line := range strings.Split(parts[1], "\n") {
		update := must.StringSplitToInts(line, ",")
